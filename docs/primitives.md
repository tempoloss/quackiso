...
The line-by-line annotations behind these entries live in [`primitives.code.json`](primitives.code.json): the cited ranges, the note lines, and a fingerprint of the code each one was written against. `python3 scripts/check_primitives_anchors.py` fails when a cited line no longer reads what it read when the annotation was written: it checks the first and last line of every range, every annotated line inside a snippet, and a fingerprint of the snippet text. Whether a range still brackets the construct the prose names is beyond it. The Primitives workflow runs it on every push, re-anchors a pure line shift by matching content instead of line numbers, and pushes that repair back, so only a substantive code change needs a person.
...
**Where:** `src/decimal.rs:1-7` - amount comments say money must not round-trip through `f64`; `src/lib.rs:890-908` maps money columns to DuckDB `DECIMAL`, not `DOUBLE`.
...
**Caught by:** `decimal::tests::shapes_seen_in_real_messages` in `src/decimal.rs:113-123` checks common wire shapes, trimming, signs, `.5`, and five fractional digits.
...
**Where:** `src/decimal.rs:9-17` states the ISO 18-significant-digit requirement and the `DECIMAL(38,5)` choice; `src/lib.rs:904-908` declares money columns with `decimal::WIDTH` and `decimal::SCALE`; `src/lib.rs:971-974` writes that column as `i128`.
...
**Where:** `src/lib.rs:3-80` names the twenty-nine readers, the sniffer, and their row grain; `src/stream.rs:122-124` keeps statement context outside entry subtrees; `src/pain001.rs:5-12` explains that debtor context lives on `PmtInf` and must be carried down.
...
**Where:** `src/lib.rs:253-270` reads the gzip magic and reports how much of it a short file had; `src/lib.rs:197-251` puts those bytes back in front of the file, wraps the result in a `MultiGzDecoder` when they match, and gives the source its name; `src/lib.rs:274-301` reads, classifies, and replays the bounded decompressed prefix before constructing an XML stream.
...
**Caught by:** `tests::gzip_reads_exactly_like_the_plain_file`, `tests::another_reader_gets_gzip_from_the_shared_source`, and `tests::a_broken_gzip_fails_instead_of_panicking` in `src/lib.rs` cover plain/gzip equivalence, concatenated members, shared-source reuse, and broken streams. `tests::xml_reader_rejects_markup_free` pins the same plain and gzip prefix error, while `membound::markup_free_plain_and_gzip_rejection_is_bounded` in `src/membound.rs` compares 128 KiB and 8 MiB inputs under the tracking allocator. MT and sniffer regressions run in the complete Rust suite.

### MT is bounded by the message, not the subtree

SWIFT MT streams like the XML readers do, and the unit it streams is not the same size. The bound is `O(message text + one output batch)`, capped at `mt::MAX_MESSAGE_BYTES` (`src/mt.rs:49`) so that a file with no message boundary in it is refused rather than read whole. A statement writes its closing balance *after* its entries, in `:62F:`, and every row a reader emits carries that balance - so the statement-level fields are parsed first into one base row, and each entry is then parsed out of its own byte range as the output batch asks for it. Nothing per entry is retained between those rows: the walk keeps a byte offset, a line count and the one region it has open, and a statement of half a million entries costs its own text and one batch. A 64-statement file peaks where an 8-statement file does, and four times the entries in one statement costs four times the text and nothing else. The price is three passes over an entry's text - once for the statement fields, once to find its region, once to tokenize that region when its row is built - and the closing balance sitting after the entries is why the first of them cannot be dropped.

**Where:** `src/mt.rs:159-165` refuses a message that reaches the cap, naming the byte count so the message says what to look for; `src/mt.rs:663-677` is the region rule, in one place because both body walks need it; `src/mt.rs:503-547` collects the statement-level fields, skips the entry regions and keeps only how many there were; `src/mt.rs:417-461` finds the next region when a row is asked for, reading each line of the body once; `src/mt940.rs:67-75` is the open statement a reader emits from - the message text, the cursor, and the base row every entry clones.
...
**Where:** `src/lib.rs:707-721` picks the worker count — an explicit
...
parallelism, and one file is always sequential; `src/lib.rs:801-819`
...
**Where:** `src/lib.rs:724-762` gives the workers a `sync_channel` of
...
against 7.7 MiB for one — and checks the rows and the sum, since a worker that
...
**Where:** `src/lib.rs:749-756` shares one `AtomicUsize` across the
...
**Where:** `src/temporal.rs:3-7` lists the mixed wire shapes and says date-times become UTC-normalised timestamps; `src/lib.rs:890-892` states that dates keep wire precision; `src/lib.rs:1051-1067` makes camt booking/value dates `Col::Stamp`, while `src/lib.rs:1100-1108` and `src/lib.rs:1149-1154` use `Col::Date` for settlement and requested execution dates.
...
**Where:** `src/temporal.rs:67-69` defines timestamps as microseconds since the Unix epoch normalised to UTC; `src/temporal.rs:114-132` handles `Z`, `+hh:mm`, and `-hh:mm` offsets.
...
**Caught by:** `temporal::tests::timestamps_normalise_to_utc` in `src/temporal.rs:152-167` checks `Z`, `+01:00`, `-01:00`, fractional seconds, and real corpus shapes.
...
**Where:** `src/temporal.rs:57-69` documents and returns those physical values; `src/lib.rs:971-972` instantiates `write_date` as `i32` and `write_timestamp` as `i64`; `src/lib.rs:1075-1080` and `src/lib.rs:1125-1128` feed parsed temporal integers into output vectors.
...
**Caught by:** `temporal::tests::epoch_and_dates` in `src/temporal.rs:142-150` checks day counts and invalid dates; `temporal::tests::timestamps_normalise_to_utc` in `src/temporal.rs:152-167` checks microsecond counts.
...
**Caught by:** `temporal::tests::hostile_text_is_null_not_a_panic` in `src/temporal.rs:169-179` puts a multi-byte character at each of the three slice sites and a non-breaking space inside the date rather than trailing, where `trim` cannot remove it; `temporal::tests::a_day_that_does_not_exist_is_null` in `src/temporal.rs:181-190` pins 31 February, 31 April, and the 1900/2000 century pair. End to end, `testdata/camt052_report.xml` books an entry on `2026-02-31` and `test/sql/quackiso.test:131-138` asserts the column is NULL.
...
**Where:** `src/lib.rs:985-1046` generates all thirty table functions; `src/lib.rs:1006-1010` declares columns and resolves files in `bind`; `src/lib.rs:1014-1016` creates scan state in `init`; `src/lib.rs:1020-1033` pulls and writes the next batch in `func`.
...
**Where:** `src/lib.rs:920-931` writes text vectors with `insert` or `set_null`; `src/lib.rs:935-974` writes numeric vectors through raw slices, recording the missing positions in a stack bitmap so each getter runs once per row, and calls `set_null` for them afterwards; `src/lib.rs:1031` sets the chunk length after writing.
...
**Where:** `src/lib.rs:833-840` rejects URI schemes while allowing Windows drive letters, keeps only the files a glob matched, and still resolves a literal name glob refuses to compile; `docs/adr/0002-no-remote-paths.md:12-18` lists the raw filesystem calls; `docs/adr/0002-no-remote-paths.md:34-54` explains the blocker: remote opens need an active transaction, `duckdb-rs` hides the raw `duckdb_function_info`, and wrapping DuckDB's output chunk is not public.
...
**Where:** `src/lib.rs:935-959` puts the raw numeric slice in an inner scope before calling `set_null`; `src/stream.rs:49-52` converts XML events into owned actions so the borrow of `self.buf` ends before calling another `&mut self` method.
