...
**Where:** `src/decimal.rs:1-7` - amount comments say money must not round-trip through `f64`; `src/lib.rs:438-455` maps money columns to DuckDB `DECIMAL`, not `DOUBLE`.
...
**Where:** `src/decimal.rs:9-17` states the ISO 18-significant-digit requirement and the `DECIMAL(38,5)` choice; `src/lib.rs:451-455` declares money columns with `decimal::WIDTH` and `decimal::SCALE`; `src/lib.rs:509-512` writes that column as `i128`.
...
**Where:** `src/lib.rs:3-34` names the thirteen table functions and their row grain; `src/stream.rs:123-125` keeps statement context outside entry subtrees; `src/pain001.rs:5-12` explains that debtor context lives on `PmtInf` and must be carried down.
...
`O(batch)` here means the live output rows are bounded by one DuckDB vector batch: at most 2048 flattened rows, plus the XML event buffer and the one entry or transaction subtree currently being copied. It does not mean the parser has loaded the file; the repo records a 1.7 GB statement reading in about 2 MB resident memory (`README.md:265-266`).

**Where:** `src/lib.rs:106-107` sets `VECTOR_SIZE` to 2048; `src/lib.rs:248-272` fills a `Vec` until that size or end-of-file; `src/lib.rs:563-568` writes that batch and tells DuckDB the row count.
...
**Where:** `src/temporal.rs:3-7` lists the mixed wire shapes and says date-times become UTC-normalised timestamps; `src/lib.rs:438-440` states that dates keep wire precision; `src/lib.rs:588-604` makes camt booking/value dates `Col::Stamp`, while `src/lib.rs:637-645` and `src/lib.rs:686-691` use `Col::Date` for settlement and requested execution dates.
...
**Where:** `src/temporal.rs:49-61` documents and returns those physical values; `src/lib.rs:509-510` instantiates `write_date` as `i32` and `write_timestamp` as `i64`; `src/lib.rs:612-617` and `src/lib.rs:662-665` feed parsed temporal integers into output vectors.
...
**Where:** `src/lib.rs:522-583` generates the three table functions; `src/lib.rs:543-547` declares columns and resolves files in `bind`; `src/lib.rs:551-553` creates scan state in `init`; `src/lib.rs:557-570` pulls and writes the next batch in `func`.
...
**Where:** `src/lib.rs:466-478` writes text vectors with `insert` or `set_null`; `src/lib.rs:481-512` writes numeric vectors through raw slices and calls `set_null` for missing values; `src/lib.rs:568` sets the chunk length after writing.
...
**Where:** `src/lib.rs:402-434` rejects URI schemes while allowing Windows drive letters; `docs/adr/0002-no-remote-paths.md:12-18` lists the raw filesystem calls; `docs/adr/0002-no-remote-paths.md:34-54` explains the blocker: remote opens need an active transaction, `duckdb-rs` hides the raw `duckdb_function_info`, and wrapping DuckDB's output chunk is not public.
...
**Where:** `src/lib.rs:481-503` puts the raw numeric slice in an inner scope before calling `set_null`; `src/stream.rs:49-52` converts XML events into owned actions so the borrow of `self.buf` ends before calling another `&mut self` method.
